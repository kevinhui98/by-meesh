...
function ProposalModal({
  ...
  return (
    ...
    >
      <div
        className="bg-background max-w-2xl w-full max-h-[80vh] overflow-y-auto rounded-2xl p-8 shadow-2xl"
      ...
      >
        ...
        <div className="border-t border-border pt-4 mt-4 text-right">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Pricing includes all ingredients, supplies, and service.
          </p>
        </div>
        <button
          onClick={() => window.print()}
          className="mt-6 w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-medium text-sm hover:opacity-90 transition-opacity"
        >
          Print / Save as PDF
        </button>
      </div>
...
function ProcurementModal({
  ...
  return (
    ...
    >
      <div
        className="bg-background max-w-xl w-full max-h-[80vh] overflow-y-auto rounded-2xl p-8 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        ...
        <button
          onClick={() => window.print()}
          className="mt-6 w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-medium text-sm hover:opacity-90"
        >
          Print / Save
        </button>
      </div>
...
export default function CostSummary() {
  ...
  const [modal, setModal] = useState<
    "proposal" | "procurement" | "invoice" | "runofshow" | null
  >(null);

  const { data: event } = useGetEvent(eventId, {
    query: { queryKey: getGetEventQueryKey(eventId) },
  });
  const { data: cost, isLoading } = useGetEventCost(
    eventId,
    { margin: parseFloat(margin.toFixed(2)) },
    {
      query: {
        queryKey: getGetEventCostQueryKey(eventId, {
          margin: parseFloat(margin.toFixed(2)),
        }),
      },
    },
  ...
  return (
    <Layout>
          ...
          <div className="bg-card border border-card-border rounded-2xl overflow-hidden mb-6">
            ...
            <div className="divide-y divide-border max-h-96 overflow-y-auto">
              {cost.dishGroups.map((group, gIdx) => {
                const groupLines = cost.lines.filter(
                  (l) => l.dishName === group.dishName,
                );
                return (
            ...
            {[
              {
                ...
                icon: FileText,
                desc: "Menu + pricing for client",
              },
              {
                id: "invoice" as const,
                label: "Invoice",
                icon: Receipt,
                desc: "Formal payment request",
              },
              {
                id: "procurement" as const,
                label: "Procurement List",
                icon: ShoppingCart,
                desc: "Ingredients & supplies to buy",
              },
              {
                id: "runofshow" as const,
                label: "Run of Show",
                icon: ClipboardList,
                desc: "Event timeline & prep notes",
              },
      ...
      {modal === "invoice" && event && cost && (
        ...
        >
          <div
            className="bg-background max-w-xl w-full rounded-2xl p-8 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="mt-4 text-xs text-muted-foreground">
              Payment due within 14 days of event date.
            </div>
            <button
              onClick={() => window.print()}
              className="mt-6 w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-medium text-sm hover:opacity-90"
      ...
      {modal === "runofshow" && event && menu && (
        ...
        >
          ...
          >
                  ...
                  <div key={course} className="mb-6">
                    ...
                    {dishes.map((d) => (
                      <div key={d.id} className="mb-4 p-3 bg-muted rounded-xl">
                        ...
                        {d.dish.flatware && (
                          <div className="text-xs text-muted-foreground mt-1">
                            <span className="font-medium">Flatware: </span>
                            {d.dish.flatware}
                          </div>
                        )}
                      </div>
            ...
            <button
              onClick={() => window.print()}
              className="mt-4 w-full bg-primary text-primary-foreground py-2.5 rounded-xl font-medium text-sm hover:opacity-90"
            >
              Print / Save
            </button>
          </div>
